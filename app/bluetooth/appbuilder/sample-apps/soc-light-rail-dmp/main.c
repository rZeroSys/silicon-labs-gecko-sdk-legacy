...
struct responseData_t{
  ...
  uint8_t shortName[DEVNAME_LEN - 1]; /**< Shortened Local Name (without terminating null) */
...
static struct responseData_t responseData = {
  ...
  0x01, /* type */
  0x04 | 0x02, /* Flags: LE General Discoverable Mode, BR/EDR is disabled. */
  DEVNAME_LEN,        // length of local name (incl type)
...
static void enableBleAdvertisements(void)
{
  ...
  uint16_t devId;
  devId = *((uint16_t*)demo.ownAddr.addr);
  ...
  Mem_Copy(((uint8_t*)&responseData) + 5, devName, strlen(devName));
...
static void demoAppTask(void *p_arg)
{
  ...
  snprintf(devIdStr, DEV_ID_STR_LEN, "DMP:%04X", *((uint16_t*)demo.ownAddr.addr));
