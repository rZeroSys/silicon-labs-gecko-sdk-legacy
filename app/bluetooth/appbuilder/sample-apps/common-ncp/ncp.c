...
void ncp_handle_command()
{
  ...
  if (ncp_command_received()) {
    ...
    if (BGLIB_MSG_ID(*cmd_header) == gecko_cmd_user_message_to_target_id) {
      ...
      uint8array *payload = (uint8array*) &rx_queue.data[BGLIB_MSG_HEADER_LEN];
...
static uint16_t get_buf_reserve_for_response()
{
  return NCP_CMD_SIZE / NCP_BUF_SIZE + (NCP_CMD_SIZE % NCP_BUF_SIZE) ? 1 : 0;
}

uint32_t ncp_get_transmit_space_for_events()
{
  return (tx_queue->size - tx_queue->used - get_buf_reserve_for_response()) * NCP_BUF_SIZE;
}

static bool ncp_enqueue(ncp_queue* queue, uint8_t* buf, uint32_t len, uint8_t rsp_not_evt)
{
  uint16_t available_size = queue->size - (rsp_not_evt ? 0 : get_buf_reserve_for_response());
