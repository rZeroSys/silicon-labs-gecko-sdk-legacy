...
#define BLUETOOTH_CREATE_VALIDRANGE(min, max) ((uint16_t)(((uint16_t)(max) << 8) | ((uint16_t)(min))))
        ...
        switch (BGLIB_MSG_ID(bluetooth_evt->header)) {
          ...
          case gecko_evt_gatt_server_user_read_request_id:
            if (gattdb_PER == bluetooth_evt->data.evt_gatt_server_user_read_request.characteristic) {
              ...
              ret = menuItemValueGet(eAppMenu_SetPhy, &miv);
              APP_ASSERT_DBG(ret, ret);
              // Send response to read request.
              gssurrrsp = gecko_cmd_gatt_server_send_user_read_response(
                bluetooth_evt->data.evt_gatt_server_user_read_request.connection,
              ...
              ret = menuItemValueGet(eAppMenu_SetChannel, &miv);
              APP_ASSERT_DBG(ret, ret);
              // Send response to read request.
              gssurrrsp = gecko_cmd_gatt_server_send_user_read_response(
                bluetooth_evt->data.evt_gatt_server_user_read_request.connection,
              ...
              ret = menuItemValueGet(eAppMenu_SetRFMode, &miv);
              APP_ASSERT_DBG(ret, ret);
              // Send response to read request.
              gssurrrsp = gecko_cmd_gatt_server_send_user_read_response(
                bluetooth_evt->data.evt_gatt_server_user_read_request.connection,
              ...
              ret = menuItemValueGet(eAppMenu_SetRFFrequency, &miv);
              APP_ASSERT_DBG(ret, ret);
              // Send response to read request.
              gssurrrsp = gecko_cmd_gatt_server_send_user_read_response(
                bluetooth_evt->data.evt_gatt_server_user_read_request.connection,
              ...
              ret = menuItemValueGet(eAppMenu_SetRFPA, &miv);
              APP_ASSERT_DBG(ret, ret);
              // Send response to read request.
              gssurrrsp = gecko_cmd_gatt_server_send_user_read_response(
                bluetooth_evt->data.evt_gatt_server_user_read_request.connection,
              ...
              ret = menuItemValueGet(eAppMenu_SetIDDest, &miv);
              APP_ASSERT_DBG(ret, ret);
              // Send response to read request.
              gssurrrsp = gecko_cmd_gatt_server_send_user_read_response(
                bluetooth_evt->data.evt_gatt_server_user_read_request.connection,
              ...
              ret = menuItemValueGet(eAppMenu_SetIDSrc, &miv);
              APP_ASSERT_DBG(ret, ret);
              // Send response to read request.
              gssurrrsp = gecko_cmd_gatt_server_send_user_read_response(
                bluetooth_evt->data.evt_gatt_server_user_read_request.connection,
              ...
              ret = menuItemValueGet(eAppMenu_SetPktsLen, &miv);
              APP_ASSERT_DBG(ret, ret);
              // Send response to read request.
              gssurrrsp = gecko_cmd_gatt_server_send_user_read_response(
                bluetooth_evt->data.evt_gatt_server_user_read_request.connection,
              ...
              ret = menuItemValueGet(eAppMenu_SetMAWindow, &miv);
              APP_ASSERT_DBG(ret, ret);
              // Send response to read request.
              gssurrrsp = gecko_cmd_gatt_server_send_user_read_response(
                bluetooth_evt->data.evt_gatt_server_user_read_request.connection,
              ...
              ret = menuItemValueGet(eAppMenu_SetLogMode, &miv);
              APP_ASSERT_DBG(ret, ret);
              // Send response to read request.
              gssurrrsp = gecko_cmd_gatt_server_send_user_read_response(
                bluetooth_evt->data.evt_gatt_server_user_read_request.connection,
              ...
              ret = menuItemValueGet(eAppMenu_RunDemo, &miv);
              APP_ASSERT_DBG(ret, ret);
              // Send response to read request.
              gssurrrsp = gecko_cmd_gatt_server_send_user_read_response(
                bluetooth_evt->data.evt_gatt_server_user_read_request.connection,
...
/*****************************************************************************
* @brief  The function changes ValidRange Descriptor of payload Characteristic
*         based on the current PHY, as:
*         - IEEE 802.15.4 Data Frame: Max payload length is 116 bytes
*         - BLE NONCONN_IND: Max payload length is 24 bytes
*
* @return None.
*****************************************************************************/
void bluetoothUpdatePayloadValidRange(void)
{
  if (PHY_IEEE_802154 == rangeTest.phy) {
    uint16_t validRange = BLUETOOTH_CREATE_VALIDRANGE(PAYLOAD_LEN_MIN, IEEE802154_PAYLOAD_LEN_MAX);
    gecko_cmd_gatt_server_write_attribute_value(gattdb_payload_valid_range, 0, 2, (uint8_t*) &validRange);
    ...
    uint16_t validRange = BLUETOOTH_CREATE_VALIDRANGE(PAYLOAD_LEN_MIN, BLE_PAYLOAD_LEN_MAX);
    gecko_cmd_gatt_server_write_attribute_value(gattdb_payload_valid_range, 0, 2, (uint8_t*) &validRange);
  } else {
    // Can not happen
    // Do nothing, i.e. doesn't change ValidRange
    APP_ASSERT_DBG(false, rangeTest.phy);
  }
}
