...
typedef enum {
  eAppMenu_ShowInfo         = 0x00, ///> Show Info menu.
...
#define MENU_ITEM_CHANGE_LIST_SIZE 16
typedef struct {
  menuItemIndex_e index;
  uint16_t value;
} MenuItemChange;

typedef struct {
  MenuItemChange items[MENU_ITEM_CHANGE_LIST_SIZE];
  uint8_t head;
  uint8_t tail;
  uint8_t size;
} MenuItemChangeList;
...
uint16_t menuItemValueGetwithValue(menuItemIndex_e mii, MenuItemValue* miv);
